...
accounts/{accountId}                        — клиент (tenant)
  ...
  commentsPollCursor    ISO-время последнего прочитанного опросом комментария
...
- **Водяной знак.** В `accounts/{id}.commentsPollCursor` хранится время последнего
  прочитанного комментария. На первом запуске курсор просто выставляется в «сейчас»
  и ничего не обрабатывается — иначе бот сгенерировал бы ответы на всю историю разом.
